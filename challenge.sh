...
# Challenge 1
gcloud compute instances create nucleus-jumphost-944 \
...
kubectl expose deployment nucleus-server --type=LoadBalancer --port 8082
...
gcloud compute instance-groups managed create nucleus-backend-group --template=nginx-template --size=2 --zone=us-east1-b


gcloud compute firewall-rules create allow-tcp-rule-160 \
    --network=default \
    --action=allow \
    --direction=ingress \
